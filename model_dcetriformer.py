...
class InputProj(nn.Module):
    def __init__(self, in_channel=4, out_channel=64, kernel_size=3, stride=1, norm_layer=None,act_layer=nn.LeakyReLU):
        super().__init__()
...
class LeWinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=5, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm,token_projection='linear',token_mlp='leff',
                 modulator=False,cross_modulator=False,triangular_flag = 0):
        super().__init__()
        self.dim = dim
...
#         print("x shape in forward",x.shape)
        ...
        if not self.triangular_flag:
            ...
            # print("triangle",x_windows.shape)
        if self.triangular_flag:
            # print("here1 triangle")
            triangular_masks = tuple(triangle_masks(shifted_x))
            x_windows  = window_partition_triangular(shifted_x, 2*self.win_size, triangular_masks)  # nw*b, window_size, window_size, c #[1, 64, 64, 180]->[16, 16, 16, 180]
            _, m, n, _ = x_windows.shape           #[b*(h_ws)*(w_ws), m, n, c]
            x_windows  = x_windows.view(-1, n, C)  #[b*(h_ws)*(w_ws)*m, n, c]  #[16, 16*16, 180]
            # print("rectangle",x_windows.shape)

#             x_windows_ = window_partition(shifted_x, self.win_size)  # nW*B, win_size, win_size, C  N*C->C
#             x_windows_ = x_windows_.view(-1, self.win_size * self.win_size, C)  # nW*B, win_size*win_size, C
#             x_windows = x_windows + x_windows_
#             xwin = [x_windows,x_windows_]
        # with_modulator
...
#         attn_windows_rect = self.attn(wmsa_in_[1],mask = attn_mask)
#         attn_windows_tri = self.attn(wmsa_in_[0],mask = attn_mask)
        # merge windows
        if self.triangular_flag:
            # print("here2 triangle")
            triangular_masks = tuple(triangle_masks(shifted_x))
#             attn_windows_ = attn_windows_rect.view(-1, self.win_size, self.win_size, C)
            attn_windows = attn_windows.view(-1, m, n, C)   #[b*(h_ws)*(w_ws), m, n, c]
            shifted_x  = window_reverse_triangular(attn_windows, 2*self.win_size, triangular_masks)  # nw*b, window_size, window_size, c #[1, 64, 64, 180]->[16, 16, 16, 180]
            shifted_x = shifted_x.view(B, H // (2*self.win_size), W // (2*self.win_size), C, 2*self.win_size, 2*self.win_size)
            shifted_x = shifted_x.permute(0, 1, 4, 2, 5, 3).contiguous().view(B,H,W,C)   #[1, 64, 64, 180]

            attn_windows = attn_windows.view(-1, self.win_size, self.win_size, C)
#             shifted_x_ = window_reverse(attn_windows_, self.win_size, H, W)  # B H' W' C
            # print("tri shape",shifted_x.shape)
            # print("rec shape",shifted_x_.shape)
            # shifted_x = shifted_x * shifted_x_
...
class BasicUformerLayer(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
        ...
        if shift_flag:
#             print("depth",depth)
#             print("input resn",input_resolution)
            self.blocks = nn.ModuleList([
                LeWinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                    num_heads=num_heads, win_size=10 if(i%2==0) else 5,
                                    ...
                                    modulator=modulator,cross_modulator=cross_modulator,triangular_flag = 0 if (i%2==0) else 1)
                for i in range(depth)])#0 if (i%2==0) else 1)
        else:
            self.blocks = nn.ModuleList([
                LeWinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                    num_heads=num_heads, win_size=10 if(i%2==0) else 5,
                                    ...
                                    norm_layer=norm_layer,token_projection=token_projection,token_mlp=token_mlp,
                                    modulator=modulator,cross_modulator=cross_modulator,triangular_flag = 0 if (i%2==0) else 1)
            for i in range(depth)])# if (i%2==0) else 1)
...
class Uformer(nn.Module):
    def __init__(self, img_size=160, in_chans=4, dd_in=3,
                 embed_dim=32, depths=[1, 2, 4, 8, 8, 8, 4, 2, 1], num_heads=[1, 2, 4, 8, 16, 16, 8, 4, 2],
                 win_size=5, mlp_ratio=4., qkv_bias=True, qk_scale=None,
...
if __name__ == "__main__":
    ...
    model_restoration = Uformer(img_size=input_size, embed_dim=16,depths=depths,
                 win_size=10 , mlp_ratio=4., token_projection='linear', token_mlp='leff', modulator=True, shift_flag=False)
...
class Discriminator(nn.Module):
    ...
    def __init__(self, input_nc=1, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        ...
        self.fc = nn.Linear(18*18,1) #for (160,160)
#         self.fc = nn.Linear(14*14,1) #for (128,128)
        self.initialize_weights()

    def initialize_weights(self):
        for item in self.model:
            if isinstance(item, nn.Conv2d):
                torch.nn.init.kaiming_uniform_(item.weight)#1-xavier_normal, 2-kaiming_uniform
...
#         print (out1.shape)
#         out2 = self.fc(out1.view(-1,14*14)) #for 128,128
#         print(out1.shape)
        out2 = self.fc(out1.view(-1,18*18)) #for 160,160 
        return out2
